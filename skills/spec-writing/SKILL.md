...
| # | 禁止行为 | 反例 | 修订动作(回 spec → §6 重检 → 写代码) |
|---|----------|------|--------------------------------------|
| 1 | 自创命名 | spec 写 `throttle_seconds`,代码用 `delay_seconds` | 回 §4 改字段名(术语表同步) → 重检 |
| 2 | 加防御性代码 | spec 未声明 `try/except`,代码自行加 fallback | 回 §5 / §6 补异常处理约束 → 重检 |
| 3 | 加未来扩展点 | spec 写 6 字段,代码加 `metadata: dict = {}` 留扩展 | 删扩展字段;如确需,回 §4 改 spec → 重检 |
| 4 | 改默认值 | spec 写 `default=UTC now`,代码用 `default=None` | 回 §4 改默认值 → 重检 |
| 5 | 改测试矩阵编号 | spec 写 TC-sub03-T-01,代码用 TC-001 | 回 §7 改编号 → 重检 |
| 6 | 改 grep 红线 | spec 写 `grep -E '"etsy"|"amazon"'`,代码用 `grep etsy` | 回 §8 改 grep 表达式 → 重检 |
| 7 | docstring 写字面量例子 | docstring 写 `"""取 value 字符串 ("etsy" / "amazon")"""` | 删字面量;如确需示例,回 §7 / §8 加测试用例 |
| 8 | spec 未声明的二次发挥 | spec 没写 `__str__`,代码加 `__str__ = str` | 回 §5 补方法签名约束 → 重检 |
...
**例外**:`假设` 段 / `约束` 段 / `变更日志` 段允许出现时间性表述(如「v2 评估」),但**v1 边界必须在 §0 元数据(状态 / 版本 / 责任人 / 关联代码路径)+ §8 非功能需求(可量化指标 + 验证方法)中钉死**;含「v2 评估」字样的句子必须同时给出 v1 当前的可执行指标,不得只挂个「v2 评估」尾巴。
