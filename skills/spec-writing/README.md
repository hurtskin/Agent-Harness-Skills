...
一个约束 AI 与人按统一骨架编写软件 Spec 的 Skill。它提供 10 段强制结构模板、零自由发挥约束清单和落地前 15 题自检，目标是让 Spec 达到「拿走文档直接复刻代码」的精度，消除实施阶段的自创空间。
...
```text
用 spec-writing 的 15 题自检评审这份 spec 是否留有自由发挥空间。
```
...
1. 按触发意图加载 10 段骨架，逐段落 spec（术语表 → 验收场景 → 契约 → 数据模型 → 规则 → 错误码 → 覆盖矩阵 → 非功能 → 假设约束）
2. 对照 11 维度钉死清单逐项确认，实施阶段必须的每个细节都写入 spec 正文
3. 执行落地前 15 题自检并记录到 spec 同目录 `self-check.md`
...
- `SKILL.md`：完整的 10 段结构模板、11 维度钉死清单、15 题自检和 SPEC-001 完整示例
