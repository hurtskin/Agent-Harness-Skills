...
> 参考实现：`scripts/validate_handoff.{ps1,sh}` 提供下述步骤 2、3、5 的「安全 YAML 解析 + 确定性机器校验 + SHA-256 字节摘要」原语，双语言等价、同失败语义；步骤 1、4 的候选写入与原子替换属宿主事务编排，不在脚本职责内。
...
交接文档是交接过程的临时产物。接任方完成接管并确认不再需要后，应删除该文档（见 §8 启动提示词第 5 步），避免过期交接文档残留；删除前说明影响。
...
5. 完成接管并确认本交接文档不再需要后，删除该文档（<absolute-path-to-handoff>）；删除前向用户说明影响，确需保留时说明保留理由。
```
